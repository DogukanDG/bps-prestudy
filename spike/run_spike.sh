...
build() {
    ...
    local built
    built="$(find "$SCYLLA_SRC/target" -maxdepth 1 -name '*.jar' ! -name '*sources*' ! -name '*tests*' | head -1)"
...
run_one() {
    ...
    # Scylla.java reads --output into a variable it never assigns to the field
    # that SimulationManager.run() actually checks, so the flag is silently
    # ignored and the real output always lands next to global_config.xml as
    # an autogenerated output_<timestamp>/ folder. Snapshot what's there
    # beforehand so we can find the one this run produced.
    local before after new_dir
    before="$(find "$HERE/$ds" -maxdepth 1 -type d -name 'output_*' 2>/dev/null)"
    ...
    after="$(find "$HERE/$ds" -maxdepth 1 -type d -name 'output_*' 2>/dev/null)"
    new_dir="$(comm -13 <(echo "$before" | sort) <(echo "$after" | sort) | head -1)"
    [ -n "$new_dir" ] && mv "$new_dir" "$out"
